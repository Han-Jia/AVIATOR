...
class ConvNet(nn.Module):
    ...
    def __init__(self, n_class, x_dim=3, hid_dim=64, z_dim=64):
        super().__init__()
        self.num_layers = 4
        ...
        # classifier layer
        self.add_module('FC', nn.Linear(z_dim, n_class))   
    ...
    def forward(self, x, params = None, embedding = False):
        ...
        if embedding:
            ...
            logits = F.linear(output, weight=params['FC.weight'.format(self.num_layers,0)],
                              bias=params['FC.bias'.format(self.num_layers,0)])     
            return logits
